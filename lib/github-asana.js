...
function getTaskActions(ref, commits) {
  var tasks=[];
  for (var i in commits) {
    ...
    ,regex_id   = /(#|app\.asana\.com\/[\/\w]+\/)(\d+)/i
    ,regex_stop = /\w(\.)/i
    ,words    = commits[i].message.split(/\s+/)
    ,current_verb = ''
    ,current_id   = ''
        ...
        for (var sw in sub_words){
            ...
            if (id !== null) {
                ...
                tasks.push({
                    ...
                    ,verb:'see'
                    ,id:current_id
                    ,message:commits[i].author.username + ' - commit pushed to ' + ref +':\n'+commits[i].id.slice(0,7)+' '+commits[i].message+'\n'+commits[i].url
                });
            } else if (verb !== null) {
                current_verb = verb[1];
...
exports.index = function(req, res){
  ...
  var ref = payload.repository.name + payload.ref.replace(/refs\/(heads|tags)/, '');
  var actions = getTaskActions(ref, payload.commits);
